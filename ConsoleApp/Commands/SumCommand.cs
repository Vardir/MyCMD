    ...
    public class SumCommand : Command
    {
        ...
        protected override ExecutionResult Execute(ExecutionResult input)
        {
            if (input.isSuccessfull && queryItems.Count != 1)
                return ExecutionResult.Error("sum.error: the command expects 1 argument");
            else if (!input.isSuccessfull && queryItems.Count != 2)
                return ExecutionResult.Error("sum.error: the command expects 2 arguments");
            ...
            {                
                ...
                {
                    return ExecutionResult.Error("sum.error: the first command argument is not a number");
                }
                var (success, number) = GetNumber(queryItems.First.Value);
                if (!success)
                    return ExecutionResult.Error("sum.error: the second command argument is expected to be a number");
                right = number;
            }
            else
            {
                var (success, number) = GetNumber(queryItems.First.Value);
                if (!success)
                    return ExecutionResult.Error("sum.error: the first command argument is expected to be a number");
                left = number;
